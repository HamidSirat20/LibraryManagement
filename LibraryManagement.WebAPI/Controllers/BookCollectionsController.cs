...
{
    private readonly IBookCollectionsService _bookCollectionService;
    private readonly ILogger<BookCollectionsController> _logger;

    public BookCollectionsController(IBookCollectionsService bookCollectionService, ILogger<BookCollectionsController> logger)
    {
    ...
    public async Task<ActionResult<BookReadDto>> GetBookCollectionAsync([ModelBinder(BinderType = typeof(ArrayModelBinder))][FromRoute] IEnumerable<Guid> bookIds)
    {
        ...
        try
        {
            ...
            var books = await _bookCollectionService.GetBookCollectionsAsync(bookIds);
